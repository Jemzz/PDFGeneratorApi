...
{
    public class JISATransferReport : IReport
    {
        private readonly IMapper _mapper;
        private readonly ITransferReportService _isaTransferService;
        public readonly IWebHostEnvironment _hostingEnvironment;

        public JISATransferReport(IWebHostEnvironment hostingEnvironment, IMapper mapper, ITransferReportService isaTransferService)
        {
        ...
        public async Task<IBaseModel> GetReport(ReportsParameters parameters)
        {
            ...
            if (parameters.IsaTransferId != 0)
            {
                ...
                if (isaTransfer == null)
                {
                    model.ErrorList.Add("Missing JISA Transfer data");
                }
                ...
                if (!model.ErrorList.Any())
                {
                    ...
                    model.Css = css;
                    model.ViewPath = "JISATransferReport";
                    model.LayoutPath = "~/Views/Shared/_Layout.cshtml";
                    model.FileName = $"JISATransfer_Report_{DateTime.UtcNow:yyyyMMMMdd_HHmmss}.pdf";
                    model.ReportName = "JISA Transfer Report";
                }
            }
            else
            {
